	...
	<div class="jumbotron rezgo-booking"> 
		...
		<div class="row rezgo-confirmation-head">
			...
			<div class="center-block">
				<button class="btn btn-lg rezgo-btn-print" onclick="window.open('<?=$site->base?>/complete/<?=$site->encode($trans_num)?>/print', '_blank'); return false;">
				...
				<div class="col-md-4 col-sm-12">
					...
					<? if($booking->status == 1 OR $booking->status == 4) { ?>
						<? $domain = $site->getDomain(); ?>
							...
							<a href="http://<?=$domain?>.<?=$role?>rezgo.com/tickets/<?=$site->encode($booking->trans_num)?>" class="btn btn-lg rezgo-btn-print-voucher btn-block" target="_blank">Print <? echo ((string) $booking->ticket_type == 'ticket') ? 'Tickets' : 'Voucher' ?></a>
					<? } ?>
				</div>
...
<? if (DEBUG) { ?><pre><?var_dump($booking);?></pre><? } ?>
