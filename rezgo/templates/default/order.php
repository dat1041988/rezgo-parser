	...
	<div class="jumbotron rezgo-booking">
		...
		<? } else { ?>
			...
			<? foreach($cart as $item) { ?>
				...
				<div class="row rezgo-form-group rezgo-cart-title-wrp">
					<div class="col-lg-9 col-sm-8 col-xs-12 rezgo-cart-title">
						...
						<? if(in_array((string) $item->date_selection, $date_types)) { ?>
							<? $data_book_date = date("Y-m-d", (string)$item->booking_date); ?>

							<label>Date: <span class="lead"><?=date((string) $company->date_format, (string) $item->booking_date)?></span></label>
						<? } else { ?>
							<? $data_book_date = date('Y-m-d', strtotime('+1 day')); // open date ?>
						...
						<div class="rezgo-order-memo rezgo-order-date-<?=date('Y-m-d', (string) $item->booking_date)?> rezgo-order-item-<?=$item->uid?>"></div>
					</div>

					<? if($site->getCartState()) { ?>
						<div class="col-lg-3 col-sm-4 col-xs-12">
							<div class="col-sm-12 rezgo-btn-cart-wrp">
								<button type="button" data-toggle="collapse" class="btn rezgo-btn-default btn-block rezgo-pax-edit-btn" data-order-item="<?=$item->uid?>" data-order-com="<?=$item->com?>" data-cart-id="<?=$item->cartID?>" data-book-date="<?=$data_book_date;?>" data-target="#pax-edit-<?=$item->cartID?>">Edit Guests</button>
							</div>
				...
				<div class="row rezgo-form-group rezgo-cart-table-wrp" id="rezgo-cart-pricing-<?=$item->uid?>">
					<div class="col-xs-12">
						<table class="table table-bordered table-striped rezgo-billing-cart table-responsive">
							...
							<? } else { $cart_total += (float) $item->sub_total; } ?>
				...
				<div id="pax-edit-scroll-<?=$item->cartID?>" class="rezgo-cart-edit-wrp"></div>
			...
			<div class="row rezgo-order-total-wrp">
