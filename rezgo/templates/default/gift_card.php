								...
								<div class="col-xs-12">
									<div class="form-group">
										<label for="billing_amount" class="control-label">
											<span>Gift Card Value in <?=$company->currency_symbol?></span>
										</label>

										<select id="rezgo-billing-amount" class="form-control" name="billing_amount">
											<option value="50" selected><?=$site->formatCurrency(50)?></option>
											<option value="100"><?=$site->formatCurrency(100)?></option>
											<option value="150"><?=$site->formatCurrency(150)?></option>
											<option value="250"><?=$site->formatCurrency(250)?></option>
							...
							<div class="row">
								...
								<div class="col-xs-12 col-sm-4">
									<div class="form-group">
										...
										<select id="billing_stateprov" class="form-control" style="<?=(($companyCountry != 'ca' && $companyCountry != 'us' && $companyCountry != 'au') ? 'display:none' : '')?>" ></select>

										<input id="billing_stateprov_txt" class="form-control" name="billing_stateprov" type="text" value="" placeholder="State/Province" style="<?=(($companyCountry != 'ca' && $companyCountry != 'us' && $companyCountry != 'au') ? '' : 'display:none')?>" />
									</div>
								</div>
							</div>
							...
							<div class="row">
								<div class="col-xs-12">
									<div id="rezgo-credit-card-container">
										...
										<iframe scrolling="no" frameborder="0" name="gift_payment" id="rezgo-gift-payment" src="<?=$site->base?>/booking_payment.php"></iframe>
									...
									<div class="form-group">
										...
										<div id="rezgo-terms-panel" class="collapse">
											<div class="collapse-wrapper">
												<?=($site->getPageContent('terms')) ? $site->getPageContent('terms') : 'Gift card terms goes here..' ?>
											</div>
										</div>
...
<? if (!$site->isVendor() && $site->getGateway()) { ?>
	...
	<? if (in_array($site->getCompanyCountry(), array('ca', 'us', 'au'))) { ?>
		$('#billing_stateprov').addOption(<?=$site->getCompanyCountry();?>_states, false);
		$('#billing_stateprov_txt').val($('#billing_stateprov').val());
	<? } ?>
	...
	function check_card_token() {
		...
		if (card_token == '') {
			...
			$purchaseForm.ajaxSubmit({
				url: '<?=$site->base?>/gift_card_ajax.php', 
				data: {
...
var gcCur = "<?=$company->currency_symbol?>";
