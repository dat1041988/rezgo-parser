		...
		<? foreach ($site->getBookings('q='.$trans_num) as $booking) { ?>
			...
			<div class="row rezgo-confirmation-head">
				<? if($booking->status == 1 OR $booking->status == 4) { ?>
					...
					<p class="rezgo-confirm-complete"><span>Click on the button below for your printable <? echo ((string) $booking->ticket_type == 'ticket') ? 'ticket' : 'voucher' ?>.</span></p>
				<? } ?>
				...
				<div class="center-block">
					...
					<? if($booking->status == 1 OR $booking->status == 4) { ?>
						<button class="btn btn-lg rezgo-btn-print-voucher" onclick="window.open('http://<?=$site->getDomain();?>.<?=$role?>rezgo.com/tickets/<?=$site->encode($trans_num)?>', '_blank'); return false;"><i class="fa fa-print fa-lg"></i>&nbsp;Print <? echo ((string) $booking->ticket_type == 'ticket') ? 'Tickets' : 'Voucher' ?></button>
					<? } ?>
				</div>
			</div>
			...
			<div class="row rezgo-form-group rezgo-confirmation-detail">
				<div class="col-sm-12">
					<h3 class="rezgo-book-name">
						<span><?=$booking->tour_name?> - <?=$booking->option_name?></span>

						<? if((string) $booking->date != 'open') { ?>
							<div class="rezgo-add-cal">
								<div class="rezgo-add-cal-cell">
									<a href="https://feed.rezgo.com/b/<?=$booking->trans_num?>"><i class="fa fa-calendar"></i>&nbsp;Add to my calendar</a>
								</div>
							</div>
						<? } ?>
					</h3>
						...
						<? if(isset($booking->expiry)) { ?>
							<tr>
								...
								<? if((int) $booking->expiry !== 0) { ?>
									<td class="rezgo-td-data"><?=date((string) $company->date_format, (int) $booking->expiry)?></td>
								<? } else { ?>
			...
			<div class="row rezgo-form-group rezgo-confirmation-additional-info">
						...
						<table border="0" cellspacing="0" cellpadding="2" class="rezgo-table-list">
							...
							<? foreach ($site->getBookingPassengers() as $passenger ) { ?>
								<tr class="rezgo-receipt-pax">
									<td class="rezgo-td-label"><?=$passenger->label?></td>
									<td class="rezgo-td-data">(<?=$passenger->num?>)</td>
								</tr>
				...
				<div class="col-md-5 col-xs-12 rezgo-company-info">
					...
					<? if($site->exists($booking->rid)) { ?>
						...
						<p><? if($site->exists($booking->rezgo_gateway)) { ?>
								Rezgo.com<br />
