...
<html>
	...
	<body class="rezgo-booking-payment-body">
		...
		<form method="post" id="payment" action="https://process.rezgo.com/form">
			<input type="hidden" name="return" value="https://<?=$_SERVER['HTTP_HOST'].$site->base?>/booking_payment.php?rezgoAction=return&">
			...
			<div id="payment_card_info" class="container-fluid">
				<div class="row">
					<div class="form-group col-xs-12">
						...
						<input type="text" class="form-control" id="name" name="name" value="<?=$site->requestStr('name')?>" placeholder="Name on Credit Card" required />
					</div>

					<div class="form-group col-xs-12">
						<label for="pan" class="control-label">Card Number</label>

						<input type="text" class="form-control" id="pan" name="pan" value="<?=$site->requestStr('pan')?>" placeholder="Credit Card Number" required />
					</div>
					...
					<? if($site->getCVV()) { ?>
						<div class="form-group col-sm-6 col-xs-12">
							<label for="rezgo-cvv" class="control-label" id="rezgo-cvv-label">
								...
								<a href="javascript:void(0);" onclick="javascript:window.open('<?=$site->path?>/img/cvv_cards.png',null,'width=600,height=300,status=no,toolbar=no,menubar=no,location=no');">
