...
<? if ($options) { ?>
	...
	<span class="rezgo-date-memo rezgo-calendar-date-<?=$_REQUEST['date']?>"></span>
	...
	<div class="panel-group" id="rezgo-select-option-<?=$option_num?>">
		...
		foreach($options as $option) { ?>
			...
			if ((($_REQUEST['type'] == 'calendar' || $_REQUEST['type'] == 'single') && (string) $option->date['value'] != 'open') 
				...
				<div class="panel panel-default<?=$panel_unclass?>">
					<script>
						...
						function check_<?=$option_num.'_'.$sub_option?>() {
							...
							if(err) {
								...
								<? } else { ?>
									$('#error_mobile_text_<?=$option_num.'_'.$sub_option?>').html(err);
									$('#error_mobile_text_<?=$option_num.'_'.$sub_option?>').slideDown().delay(2000).slideUp('slow');
								<? } ?>
								return false;
							}
						}
						...
						<div class="panel-body">
							<? if ($option->date->availability != 0) { ?>
								<span class="rezgo-option-memo rezgo-option-date-<?=$_REQUEST['date']?>"></span>
								...
								<form class="rezgo-order-form" name="checkout_<?=$option_num.'_'.$sub_option?>" id="checkout_<?=$option_num.'_'.$sub_option?>" action="<?=$site->base?>/order">
									...
									<div class="row"> 
										<div class="col-xs-12">
											...
											<div class="text-danger rezgo-option-error" id="error_mobile_text_<?=$option_num.'_'.$sub_option?>" style="display:none;"></div>
										</div><!-- end col-sm-8-->
